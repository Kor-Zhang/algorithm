 ...
 * <pre>
 * 重点理解:
 * 1.总的思想是:在"所有的已访问顶点的子顶点"中找寻最小权重(从当前顶点到起始顶点)的顶点加入"已访问序列";
 * 2.每个未访问顶点的权值是该顶点到起始顶点的总权值;
 * 3.当每次访问一个新的节点成功后,需要对访问节点更新.
 * 	具体的更新操作:
...
public class Dijkstra implements ShortestPath<DijkstraNode> {
	...
	@Override
	public List<DijkstraNode> produce(Integer[][] adjacentMatrix, Integer startV) {
		// 初始化以访问节点集合和为访问节点集合
		...
		while (unVisited.size() != 0) {
			...
			for (int j = 0; j < adjacentMatrix[minD.vIndex].length; j++) {
				...
				DijkstraNode n0 = new DijkstraNode(
						adjacentMatrix[minD.vIndex][j] + minD.weight, j);// 初始化一个dijkstra的节点,并且更新当前顶点的度为该顶点到其实定点的总权值;
				if (!visited.contain(n0)) {// 如果没有被拜访过
					if (unVisited.contain(n0)) {// **重点**如果未拜访序列包含该顶点,说明该顶点也是其他节点的度,那么比较这两条路径那个的权最小,更新为访问序列中的该索引的顶点的内容;
